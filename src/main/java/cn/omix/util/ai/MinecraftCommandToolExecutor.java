...
final class MinecraftCommandToolExecutor implements AiToolExecutor {
    ...
    void reset() { containerTools.reset(); }
    ...
    public CompletableFuture<String> execute(AiToolCall call) {
        ...
        if (client == null) {
            return CompletableFuture.failedFuture(new IllegalStateException("The Minecraft client is unavailable."));
        }
        ...
        try {
            ...
            if (AiContainerTools.supports(toolName)) {
                ...
                pending.whenComplete((content, error) -> {
                    if (error == null) result.complete(content.toString());
    ...
    private void executeAndCapture(
        ...
        try {
            ...
            result.completeExceptionally(new IllegalStateException(
                    captured.isEmpty() || captured.startsWith("(No new chat") ? detail : captured + "\n" + detail, exception));
            return;
        }

        AiChatCapture.Capture completedCapture = capture;
        result.whenComplete((value, error) -> {
            if (result.isCancelled()) AiChatCapture.finish(completedCapture);
        });
