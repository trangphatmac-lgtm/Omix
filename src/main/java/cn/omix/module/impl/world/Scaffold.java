...
public class Scaffold extends Module {
    ...
    private final BoolValue movementFix = new BoolValue("Movement Fix", false, () -> !rotationMode.is("On tick"));
    ...
    private float[] onTickServerRotations;
    ...
    public void onUpdate(UpdateEvent event) {
        ...
        if (!skipTowerPlacement && canPlace && data != null) {
            if (rotationMode.is("On tick")) {
                if (delayTimer.hasTimeElapsed(delay.getValue()) && placeOnTick()) {
                    delayTimer.reset();
                }
                return;
            }
            ...
            if (this.rayCast.getValue()) {
                // The render crosshair can use an interpolated rotation and an
                // older target. Trace from the current eyes using the applied aim.
                float yaw = mc.player.getYaw();
                float pitch = mc.player.getPitch();
                if (RotationManager.isRotating()) {
                    yaw = RotationManager.currentRotations[0];
                    pitch = RotationManager.currentRotations[1];
                }
                BlockHitResult hit = ClientRayTraceUtil.raycastBlock(yaw, pitch, PLACEMENT_REACH);
                rayCast = hit != null && hit.getType() == HitResult.Type.BLOCK
                        && hit.getBlockPos().equals(data.blockPos());
            }
    ...
    private float[] getOnTickRotations() {
        ...
        float[] base = RotationManager.isRotating() ? RotationManager.currentRotations
                : new float[]{mc.player.getYaw(), mc.player.getPitch()};
        // LiquidBounce Rotation.normalize(): quantize the wrapped delta to the
    ...
    private BlockHitResult getOnTickHit(float yaw, float pitch) {
        BlockHitResult hit = ClientRayTraceUtil.raycastBlock(yaw, pitch, PLACEMENT_REACH);
    ...
    public void onPacket(PacketEvent event) {
        if (event.getType() == PacketEvent.Type.Send && !event.isCancelled()
                && event.getPacket() instanceof PlayerMoveC2SPacket packet && packet.changesLook()) {
            onTickServerRotations = new float[]{packet.getYaw(0.0F), packet.getPitch(0.0F)};
        }
    }

    private boolean matchesOnTickServerRotation(float yaw, float pitch) {
        if (onTickServerRotations == null) {
            ClientPlayerEntityAccessor player = (ClientPlayerEntityAccessor) mc.player;
            onTickServerRotations = new float[]{player.getLastYaw(), player.getLastPitch()};
        }
        return onTickServerRotations[0] == yaw && onTickServerRotations[1] == pitch;
    }

    private void sendOnTickRotation(float yaw, float pitch) {
    ...
    private boolean placeOnTick() {
        if (mc.player == null || mc.world == null || mc.interactionManager == null || data == null) return false;

        float[] target = getOnTickRotations();
        BlockHitResult hit = getOnTickHit(target[0], target[1]);
        if (hit == null) return false;

        if (!matchesOnTickServerRotation(target[0], target[1])) {
            sendOnTickRotation(target[0], target[1]);
        }
        try {
