...
class AiContainerToolsTest {
    ...
    void validatesDiscoveryAndPositionBeforeAccessingClient() {
        ...
        assertInstanceOf(IllegalArgumentException.class, assertThrows(java.util.concurrent.CompletionException.class,
                () -> new MinecraftCommandToolExecutor().execute(new AiToolCall(
                        "invalid", "getnearbycontainer", "{\"range\":21}")).join()).getCause());
    }
