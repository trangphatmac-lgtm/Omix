...
向服务器提供自定义的偏航和俯仰方向，支持固定、偏移、随机和旋转表现。旋转以优先级 900 提交 silent 请求并立即应用。
