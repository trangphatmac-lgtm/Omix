...
使用物品时取消移动减速，可保持疾跑。Grim 参考提供的 NoSlow 逆向模型及共享队列策略，保留实际使用手，通过使用状态确认、接收包缓冲和移动包边界恢复；支持食物、饮用药水、弓及未蓄能弩。主副手长按可连续食用，每次完整回放物品栏同步后自动开始下一次；位置纠正及时处理。Grim 使用期间锁定快捷栏，双手均为消耗品、已蓄能弩、方块交互和不支持的物品保留原版减速。详见 [Grim 实现与验证说明](noslow-grim.md)。

源码：`src/main/java/cn/omix/module/impl/move/NoSlowDown.java`。

| 配置项 | 简介 | 类型、默认值与限制 |
| --- | --- | --- |
| Mode | Vanilla 直接取消物品使用减速；Grim 在收到与本次使用手匹配的使用状态后取消减速，使用结束后按顺序完整回放缓冲包，不额外交换主副手。默认仍为 Vanilla。 | 模式；默认 Vanilla；可选 Vanilla / Grim |
